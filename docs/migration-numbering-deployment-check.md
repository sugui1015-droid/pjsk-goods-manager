...
| D | 无 `schema_migrations` 且步骤 6 为零行 | 全新安装。确认连接目标无误后启动后端，全部迁移按序自动执行。启动预算是分开的：连接 10 秒、迁移 2 分钟（`backend/main.go`），全新库全量迁移在这个预算内即使对远程库也有充裕余量。若迁移仍然超时，中断是安全的（单事务回滚、不半提交），重启后端会从断点继续。 |
