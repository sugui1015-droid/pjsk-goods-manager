...
- **Caddy 网关实际部署（2026-07-16）**：本机以 NSSM 注册 `pjsk-caddy`（Caddy v2.11.4，LocalService，依赖 `pjsk-backend`），监听 **8081**（80 被本机 IIS/W3SVC 占用），静态前端 `D:\PJSK-Deploy\frontend` + SPA 回退 + 反代 `/api/*`、`/health`；防火墙 `PJSK Caddy HTTP LAN` 仅放行 TCP 8081（Private+LocalSubnet+限定 caddy.exe）。验收（首页/静态资源/SPA 回退/API 代理/健康检查/启停与异常恢复）全部通过，跨设备访问与 HTTPS 仍待办；记录见 [docs/development-logs/2026-07-16-internal-caddy-deployment.md](../../docs/development-logs/2026-07-16-internal-caddy-deployment.md)。`caddy-winsw.xml.example` 为 WinSW 形态示例，本机未采用。
