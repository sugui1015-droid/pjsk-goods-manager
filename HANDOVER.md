...
1. Migration numbering already continued from `0013` onward (now at `0019`), leaving the historical duplicate-`0005` pair untouched; the only remaining decision is whether to ever clean up that pair, which still requires its own reviewed pass (see Known Issues).
...
9. Production deployment: domain, HTTPS, backup strategy.
   ...
   - Windows service deployment: guide and example configs done ([docs/windows-service-deployment.md](docs/windows-service-deployment.md), `deploy/windows-service/`). WinSW is recommended (NSSM as fallback); backend builds to `backend/bin/pjsk-backend.exe` via `go build -trimpath -o .\bin\pjsk-backend.exe .` (entrypoint is `pjsk/backend` root, not `cmd/`), secrets stay in an ACL-restricted `secrets\backend.env` loaded from the service working directory. No service-related code change was needed (graceful HTTP shutdown noted as an optional, non-blocking future enhancement). Still pending: actually registering/running services, creating the dedicated service account, and setting ACLs on the target host.
