...
9. Production deployment: domain, HTTPS, backup strategy.
   ...
   - Internal reverse proxy / web gateway: **the LAN HTTP stage is live (2026-07-16)**. Service `pjsk-caddy` (display name "PJSK Goods Manager Web Gateway") runs Caddy **v2.11.4** (`D:\PJSK-Service\caddy\caddy.exe`, official release verified against the published checksums) under NSSM as `NT AUTHORITY\LocalService`, start type Automatic, depends on `pjsk-backend`. It listens on **:8081** (port 80 is held by this machine's IIS/W3SVC — likely used by Siemens tooling — and was deliberately left untouched), serves the production frontend build from `D:\PJSK-Deploy\frontend` with SPA history fallback, and proxies only `/api/*` and `/health` to `127.0.0.1:8080` (`/admin/*` is a frontend route, never proxied). Config: `D:\PJSK-Service\caddy\Caddyfile` (outside the repo; sanitized template at `deploy/windows-service/Caddyfile.http-lan.example`); logs: `D:\PJSK-Runtime\logs\caddy`. Firewall: one rule "PJSK Caddy HTTP LAN" — Inbound/Allow/TCP 8081, Private profile, LocalSubnet only, bound to caddy.exe. Entry points: LAN `http://192.168.1.10:8081/`, loopback `http://127.0.0.1:8081/`, proxied health `http://192.168.1.10:8081/health`. Acceptance passed (homepage, static assets, SPA fallback for `/admin/orders` and `/query`, `/api/config` 200 JSON, unauthenticated admin API 401, no directory listing, no dev-server markers) plus stop/restart/kill-recovery reliability tests. Cross-device access **has been verified via a Windows mobile hotspot** (phone opened the gateway homepage and `/health` successfully); the original CMCC-4cXx Wi-Fi could not be used for device-to-device access due to suspected client/AP isolation (phone got `ERR_ADDRESS_UNREACHABLE`, PC could not ping the phone either — a network-side limitation, not a Caddy/firewall issue; router left untouched). A real full-machine reboot test passed (2026-07-16): with no manual intervention, `postgresql-x64-18`, `pjsk-backend` and `pjsk-caddy` all came back Running/Automatic, backend stayed loopback-only on 8080, Caddy listened on 8081, all three health/homepage checks returned 200, and the phone reached the gateway again via the Windows mobile hotspot. PostgreSQL 5432 loopback hardening is **resolved (2026-07-16)**: PostgreSQL now listens only on `127.0.0.1:5432` and `[::1]:5432` (`listen_addresses = '127.0.0.1, ::1'`), so the database is no longer directly exposed on LAN interfaces; `pjsk-backend` continues to connect via `::1`, and `pg_hba.conf` was not changed. Windows Firewall hardening is **resolved (2026-07-16)**: the Domain/Private/Public profiles (previously all OFF) are now all **Enabled** with default inbound **Block** / default outbound **Allow**, NotifyOnListen=True, LogBlocked=True, and the default log path/size untouched — applied by the gated, backed-up, rollback-capable script `D:\PJSK-Archive\firewall\Set-PjskFirewallBaseline.ps1` (outside the repo; apply exit code 0; pre-change backup kept outside the repo). PJSK's only LAN entry point is Caddy on **TCP 8081** via the existing "PJSK Caddy HTTP LAN" rule (Private profile, LocalSubnet, bound to caddy.exe); **no inbound allow rules exist for backend 8080 or PostgreSQL 5432**, which stay loopback-only. Phone access via the Windows mobile hotspot was re-verified after the firewall went live. A real full-machine reboot **after** the firewall went live was validated on 2026-07-16 (read-only, no manual service starts): all four services (`postgresql-x64-18`, `pjsk-backend`, `pjsk-caddy`, `mpssvc`) came back Running/Automatic in dependency order with no crash events, the three firewall profiles stayed Enabled with Block/Allow defaults, the PJSK rule stayed the only 8081 allow, 5432/8080 stayed loopback-only with no inbound allow rules, all three health checks returned 200, pages rendered cleanly, the READ ONLY database integrity checks (19 migrations / `0019` / exactly 1) passed, and the backend still connected via `::1 -> ::1:5432`; cross-device acceptance reuses the previously passed hotspot evidence. The CMCC-4cXx router's client/AP isolation (devices on that router cannot reach each other) is a network-side limitation independent of the firewall and PJSK. Follow-up (separate, not yet done): review/narrow the pre-existing broad third-party inbound rules observed during the investigation — IIS 80/443, Siemens SCP 443, ToDesk, AweSun, Rockwell/GX Works/licensing services and other any-address rules; if industrial software later has device-discovery/licensing/LAN issues, check that software's specific rules instead of disabling the firewall. Still pending: internal DNS/domain, HTTPS, internal-CA trust, Caddy upgrade/rollback drills, and full manual UAT of business pages. Evidence logs: [docs/development-logs/2026-07-16-internal-caddy-deployment.md](docs/development-logs/2026-07-16-internal-caddy-deployment.md), [docs/development-logs/2026-07-16-postgresql-listen-address-hardening.md](docs/development-logs/2026-07-16-postgresql-listen-address-hardening.md) and [docs/development-logs/2026-07-16-windows-firewall-hardening.md](docs/development-logs/2026-07-16-windows-firewall-hardening.md).
